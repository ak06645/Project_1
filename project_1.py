...
# your code
splits, ds_info = tfds.load(name='eurosat:2.0.0',split=['train[:80%]', 'train[80%:90%]', 'train[90%:]'], with_info=True ,as_supervised=True)
...
Label shape: {ds_info.features['label'].shape}
Number of classes: {ds_info.features['label'].num_classes}
Class names: {ds_info.features['label'].names}
Number of training examples: {ds_info.splits.total_num_examples}
''')
...
# your code
tfds.show_examples(ds_info=ds_info, ds=train_ds, rows=2, cols=5)
...
# your code
train_occurences = count_occurences(train_ds, ds_info)
val_occurences = count_occurences(val_ds, ds_info)
test_occurences = count_occurences(test_ds, ds_info)
print(f"""
train_ds:\t {train_occurences}
val_ds:\t {val_occurences}
test_ds\t {test_occurences}
""")
...
smallest entry of an image - {min}
largest entry of an image - {max}
"""))
...
get_ipython().system('module load cuda/10.0')


# In[13]:


get_ipython().system('module load lib/cudnn/7.6.1.34_cuda_10.0')


# In[16]:


get_ipython().system('source activate tf-200-GPU-py37')
...
model = keras.Sequential(
    [
        keras.layers.Flatten(input_shape=ds_info.features["image"].shape),
        keras.layers.Dense(256, activation="ReLU"),
        keras.layers.Dense(ds_info.features['label'].num_classes, activation="softmax")
    ]
)
...
model.compile(loss=keras.losses.sparse_categorical_crossentropy,
              optimizer=keras.optimizers.Adam(learning_rate=keras.optimizers.schedules.PolynomialDecay(
...
# your code
history = model.fit(train_ds, steps_per_epoch=np.ceil(sum(train_occurences)/batch_size), validation_data=val_ds, epochs=100)
...
plt.show()
print(f'True label: {demo_label} ({ds_info.features["label"].names[demo_label]})\n')

# print my predicted class probabilities and demo_label
print('My Predicted Class Probabilities:\n')
for i in range(output.shape[1]):
    print(f'{i}: {output[0][i]:.5f}')
print(f'\nMy Predicted label: {predicted_label} ({ds_info.features["label"].names[predicted_label]})')
